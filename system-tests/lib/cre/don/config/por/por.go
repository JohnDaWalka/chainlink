...
func GenerateConfigs(input cretypes.GeneratePoRConfigsInput) (cretypes.NodeIndexToConfigOverride, error) {
	...
	bootstrapNodes, err := node.FindManyWithLabel(input.DonMetadata.NodesMetadata, &cldtypes.Label{Key: cldtypes.NodeTypeKey, Value: cretypes.BootstrapNode}, node.EqualLabels)
	if err != nil {
