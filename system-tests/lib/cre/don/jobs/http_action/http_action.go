...
func GenerateJobSpecs(donTopology *types.DonTopology, httpActionBinaryPath string) (types.DonsToJobSpecs, error) {
	...
	for _, donWithMetadata := range donTopology.DonsWithMetadata {
		...
		for _, workerNode := range workflowNodeSet {
			...
			donToJobSpecs[donWithMetadata.ID] = append(donToJobSpecs[donWithMetadata.ID], jobs.WorkerStandardCapability(nodeID, types.HttpActionCapability, httpActionBinaryPath, serviceConfigTemplate))
		}
	}
