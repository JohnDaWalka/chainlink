...
func generateDonJobSpecs(
	...
	if creflags.HasFlag(donWithMetadata.Flags, types.OCR3Capability) {
		...
		bootstrapNode, bootErr := node.FindOneWithLabel(donWithMetadata.NodesMetadata, &cldtypes.Label{Key: cldtypes.NodeTypeKey, Value: types.BootstrapNode}, node.EqualLabels)
		if bootErr != nil {
