...
func StartDONsAndJD(
	...
	for donIdx, donMetadata := range topology.DonsMetadata.List() {
		...
		executableErr := makeBinariesExecutable(customBinariesPaths)
		if executableErr != nil {
			return nil, nil, pkgerrors.Wrap(executableErr, "failed to make binaries executable")
		}

		var err error
		ns, err := appendBinariesPathsNodeSpec(capabilitiesAwareNodeSets[donIdx], donMetadata, customBinariesPaths)
		if err != nil {
...
func makeBinariesExecutable(customBinariesPaths map[cre.CapabilityFlag]string) error {
	for capabilityFlag, binaryPath := range customBinariesPaths {
		...
		if _, err := os.Stat(binaryPath); os.IsNotExist(err) {
			...
			if absErr != nil {
				return pkgerrors.Wrapf(absErr, "failed to get absolute path for binary %s", binaryPath)
			}
		...
		if err := os.Chmod(binaryPath, 0755); err != nil {
			return pkgerrors.Wrapf(err, "failed to make binary %s executable for capability %s", binaryPath, capabilityFlag)
		}
	}
