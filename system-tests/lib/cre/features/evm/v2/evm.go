...
func createJobs(
	...
	for _, chainID := range chainConfig.EnabledChains {
		...
		bootInput := cre_jobs.ProposeJobSpecInput{
			...
			DONName:     don.Name,
			JobName:     fmt.Sprintf("evm-ocr3-bootstrap-%d", chainID),
		...
		if bootVerErr != nil {
			return fmt.Errorf("precondition verification failed for EVM OCR3 bootstrap job for chainID %d: %w", chainID, bootVerErr)
		}

		bootReport, bootErr := cre_jobs.ProposeJobSpec{}.Apply(*creEnv.CldfEnvironment, bootInput)
		if bootErr != nil {
			return fmt.Errorf("failed to propose EVM OCR3 bootstrap job spec for chainID %d: %w", chainID, bootErr)
		}
		...
		for _, workerNode := range workerNodes {
			...
			workerInput := cre_jobs.ProposeJobSpecInput{
				...
				Environment: cre.EnvironmentName,
				DONName:     don.Name,
				JobName:     fmt.Sprintf("evm-ocr3-worker-%d", chainID),
				ExtraLabels: map[string]string{cre.CapabilityLabelKey: flag},
				DONFilters: []offchain.TargetDONFilter{
					{Key: offchain.FilterKeyDONName, Value: don.Name},
