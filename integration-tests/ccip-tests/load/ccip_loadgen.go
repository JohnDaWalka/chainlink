...
func (c *CCIPE2ELoad) Call(_ *wasp.Generator) *wasp.Response {
	...
	fee = new(big.Int).Mul(fee, big.NewInt(10000000000))
	c.Lane.Logger.Info().Str("Source Chain", sourceCCIP.Common.ChainClient.GetNetworkName()).Str("Fee", fee.String()).Msg("Calculated fee After")
	if err != nil {
