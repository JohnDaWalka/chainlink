...
library Utils {
  ...
  function getBuildInfoFile(
    ...
    if (!stdout.toSlice().endsWith(".json".toSlice())) {
      revert(
        string(
          abi.encodePacked("Could not find build-info file with matching source code hash for contract ", contractName)
        )
      );
    }
