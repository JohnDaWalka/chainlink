...
import {Strings} from "../../../openzeppelin-solidity/v5.0.2/contracts/utils/Strings.sol";

import {ApprovalProcessResponse, ProposeUpgradeResponse} from "../Defender.sol";
import {DefenderOptions, Options} from "../Options.sol";
import {ContractInfo, Utils} from "./Utils.sol";
...
library DefenderDeploy {
  ...
  ) internal pure returns (ProposeUpgradeResponse memory) {
  ...
  ) internal returns (ApprovalProcessResponse memory) {
  ...
  function parseApprovalProcessResponse(
    string memory stdout
  ) internal pure returns (ApprovalProcessResponse memory) {
    Vm vm = Vm(Utils.CHEATCODE_ADDRESS);
