...
library Core {
   ...
   */
  function getUpgradeInterfaceVersion(
    address addr
  ...
  function buildValidateCommand(
    ...
    if (hasReferenceContract) {
      string memory referenceArg =
        hasReferenceBuildInfoDir ? opts.referenceContract : Utils.getFullyQualifiedName(opts.referenceContract, outDir);
  ...
  function _deploy(string memory contractName, bytes memory constructorData) private returns (address) {
    ...
    if (deployedAddress == address(0)) {
      revert(
        string(
          abi.encodePacked(
            "Failed to deploy contract ", contractName, ' using constructor data "', string(constructorData), '"'
          )
        )
      );
    }
