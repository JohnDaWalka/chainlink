...
contract VerifierProxy is IVerifierProxy, ConfirmedOwner, TypeAndVersionInterface {
  ...
  IVerifierProxyVerifier private s_verifier;
  ...
  function setVerifier(address verifierAddress) external onlyOwner {
    ...
    if (!IERC165(verifierAddress).supportsInterface(type(IVerifierProxyVerifier).interfaceId))
      revert VerifierInvalid(verifierAddress);

    s_verifier = IVerifierProxyVerifier(verifierAddress);
  }
  ...
  function supportsInterface(bytes4 interfaceId) external pure override returns (bool) {
    return interfaceId == type(IVerifierProxy).interfaceId;
  }
}
