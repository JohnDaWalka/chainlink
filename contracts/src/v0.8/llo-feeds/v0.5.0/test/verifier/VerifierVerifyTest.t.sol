...
contract VerifierVerifyTest is BaseTest {
  ...
  function test_verifyTogglingActiveFlagsDonConfigs() public {
    ...
    Signer[] memory signers = _getSigners(MAX_ORACLES);
    address[] memory signerAddrs = _getSignerAddresses(signers);
    s_reportContext[0] = bytes32(abi.encode(uint256(1)));
    bytes memory signedReport = _generateV3EncodedBlob(s_testReportThree, s_reportContext, signers);
    s_verifier.setConfig(bytes32(uint256(1)), signerAddrs, FAULT_TOLERANCE, new Common.AddressAndWeight[](0));
    // verifies report
    bytes memory verifierResponse = s_verifierProxy.verify(signedReport, abi.encode(native));
    ...
    s_verifier.setConfigActive(bytes32(uint256(1)), false);
    ...
    s_verifier.setConfigActive(bytes32(uint256(1)), true);
    verifierResponse = s_verifierProxy.verify(signedReport, abi.encode(native));
    assertReportsEqual(verifierResponse, s_testReportThree);
  }
  ...
  //TODO is this test still relevant? You can't have a newer config now.
  ...
  //   address[] memory signersAddrSubset1 = _getSignerAddresses(signersSubset1);
  //   // Config1
  //   s_verifier.setConfig(bytes32(uint256(1)), signersAddrSubset1, MINIMAL_FAULT_TOLERANCE, new Common.AddressAndWeight[](0));
  ...
  //   // Config2
  //   s_verifier.setConfig(bytes32(uint256(1)), signersAddrSubset2, MINIMAL_FAULT_TOLERANCE, new Common.AddressAndWeight[](0));
  ...
  //   s_verifier.setConfig(bytes32(uint256(1)), signersAddrsC, MINIMAL_FAULT_TOLERANCE, new Common.AddressAndWeight[](0));
