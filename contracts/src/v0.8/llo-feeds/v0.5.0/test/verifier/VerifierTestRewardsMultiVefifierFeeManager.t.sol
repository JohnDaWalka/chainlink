...
contract MultiVerifierBillingTests is MultipleVerifierWithMultipleFeeManagers {
  ...
  function test_multipleFeeManagersAndVerifiers() public {
    ...
    */
    ...
    //TODO Not sure if these should have different config digests
    s_verifier.setConfig(bytes32(uint256(1)), signerAddrs, FAULT_TOLERANCE, weights);
    s_verifier2.setConfig(bytes32(uint256(1)), signerAddrs, MINIMAL_FAULT_TOLERANCE, weights2);
    s_verifier3.setConfig(bytes32(uint256(1)), signerAddrs, MINIMAL_FAULT_TOLERANCE + 1, weights3);
