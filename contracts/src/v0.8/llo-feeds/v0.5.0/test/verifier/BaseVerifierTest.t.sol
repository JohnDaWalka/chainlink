...
contract BaseTest is Test {
  ...
  function setUp() public virtual {
    ...
    feeManager = new FeeManager(address(link), address(native), address(s_verifier), address(rewardManager));
...
contract MultipleVerifierWithMultipleFeeManagers is BaseTest {
  ...
  Verifier internal s_verifier2;
  Verifier internal s_verifier3;

  VerifierProxy internal s_verifierProxy2;
  VerifierProxy internal s_verifierProxy3;

  FeeManager internal feeManager2;
  ...
  function setUp() public virtual override {
    ...
    s_verifierProxy2 = new VerifierProxy();
    s_verifierProxy3 = new VerifierProxy();

    s_verifier2 = new Verifier(address(s_verifierProxy2));
    s_verifier3 = new Verifier(address(s_verifierProxy3));

    s_verifierProxy2.setVerifier(address(s_verifier2));
    s_verifierProxy3.setVerifier(address(s_verifier3));

    feeManager2 = new FeeManager(address(link), address(native), address(s_verifier), address(rewardManager));
