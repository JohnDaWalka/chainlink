...
func Test_configureOCR3Request_generateOCR3Config(t *testing.T) {
	...
	got, err := GenerateOCR3ConfigFromNodes(cfg, nodes, chain_selectors.ETHEREUM_TESTNET_SEPOLIA.Selector, focr.XXXGenerateTestOCRSecrets(), nil)
	require.NoError(t, err)
	...
	t.Run("no multiple transmitters", func(t *testing.T) {
		...
		_, err := GenerateOCR3ConfigFromNodes(cfg2, nodes, chain_selectors.ETHEREUM_TESTNET_SEPOLIA.Selector, focr.XXXGenerateTestOCRSecrets(), nil)
		require.Error(t, err)
	})
