...
func Test_OpAddAddrBookRecord(t *testing.T) {
	...
	var (
		csel uint64 = 16015286601757825753
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := operations.ExecuteOperation(
				...
				operations.WithRetryConfig(
					operations.RetryConfig[OpAddAddrBookRecordInput, OpAddAddrBookRecordDeps]{
						DisableRetry: true,
					},
				),
			)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := operations.ExecuteOperation(
				...
				operations.WithRetryConfig(
					operations.RetryConfig[OpAddDatastoreAddrRefInput, OpAddDatastoreAddrRefDeps]{
						DisableRetry: true,
					},
				),
			)
