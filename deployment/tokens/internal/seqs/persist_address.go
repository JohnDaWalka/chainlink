	...
	func(b operations.Bundle, deps SeqPersistAddressDeps, in SeqPersistAddressInput) (SeqPersistAddressOutput, error) {
		...
		if _, err := operations.ExecuteOperation(b, ops.OpAddAddrBookRecord,
			...
			operations.WithRetryConfig(
				operations.RetryConfig[ops.OpAddAddrBookRecordInput, ops.OpAddAddrBookRecordDeps]{
					DisableRetry: true,
				},
			),
		); err != nil {
		...
		if _, err := operations.ExecuteOperation(b, ops.OpAddDatastoreAddrRef,
			...
			operations.WithRetryConfig(
				operations.RetryConfig[ops.OpAddDatastoreAddrRefInput, ops.OpAddDatastoreAddrRefDeps]{
					DisableRetry: true,
				},
			),
		); err != nil {
