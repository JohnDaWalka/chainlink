...
func TestDeployOCR3CapabilitySeq(t *testing.T) {
	...
	t.Run("failure - invalid registry chain selector", func(t *testing.T) {
		...
		input := keystoneops.DeployOCR3CapabilityInput{
			...
			BootstrapperOCR3Urls: []string{"12D3KooWHfYFQ8hGttAYbMCevQVESEQhzJAqFZokMVtom8bNxwGq@"},
			BootstrapCfgs:        bootstrapNodes,
			Capabilities:         capabilities,
		}
		...
		_, err = operations.ExecuteSequence(bundle, keystoneops.DeployOCR3CapabilitySeq, deps, input)
	...
	t.Run("failure - invalid registry contract address", func(t *testing.T) {
		...
		invalidAddrRef := datastore.NewAddressRefKey(
			te.RegistrySelector,
			datastore.ContractType(changeset.CapabilitiesRegistry),
