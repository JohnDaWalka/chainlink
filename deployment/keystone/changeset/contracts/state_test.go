...
func Test_GetContractSet(t *testing.T) {
	type testCase struct {
		name        string
		giveRequest *contracts.GetContractSetsRequest
		wantResp    *contracts.GetContractSetsResponse
		wantErr     string
	}
	lggr := logger.Test(t)
	...
	tt := []testCase{
		{
			...
			giveRequest: func() *contracts.GetContractSetsRequest {
				...
				req := &contracts.GetContractSetsRequest{
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			gotResp, err := contracts.GetContractSets(lggr, tc.giveRequest)
			if tc.wantErr != "" {
				require.Error(t, err)
