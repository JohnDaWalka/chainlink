...
func createAptosAccount(t *testing.T, useDefault bool) *aptos.Account {
	if useDefault {
		...
		//privateKey := ed25519.PrivateKey(append(privateKeyBytes, defaultAddress[:]...))
		...
		//account, err := aptos.NewAccountFromSigner(&crypto.Secp256k1PrivateKey{Inner: privateKey}, defaultAddress)
		require.NoError(t, err)
		return account
	} else {
