...
func (js *spawner) stopService(jobID int32) {
	...
	if !found {
		...
		lggr.Info("Spawner.stopService - job is active and has %d services", len(aj.services))
	}
	...
	for i := len(aj.services) - 1; i >= 0; i-- {
		...
		if c, ok := service.(services.HealthReporter); ok {
			...
			sLggr.Infof("Spawner.stopService - unregistered health reporter")
		}
		sLggr.Infof("Spawner.stopService - about to close service")
		...
		sLggr.Infof("Spawner.stopService - service closed")
	}
	lggr.Infof("Spawner.stopService - all services closed")

	delete(js.activeJobs, jobID)
	lggr.Infof("Spawner.stopService - deleted job from active jobs")
}
