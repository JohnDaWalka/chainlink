...
func (r *RelayerFactory) ActualNewTON(ks coretypes.Keystore, chainCfgs RawConfigs) (map[types.RelayID]loop.Relayer, error) {
	...
	for _, chainCfg := range chainCfgs {
		...
		if cmdName := env.TONPlugin.Cmd.Get(); cmdName != "" {
			...
			// tonaRelayers[relayID] = relay.NewServerAdapter(solana.NewRelayer(lggr, chain, r.CapabilitiesRegistry))
