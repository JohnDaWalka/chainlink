...
`)
	...
	lggr := logger.Test(t)
	_, err := gateway.NewGatewayFromConfig(parseTOMLConfig(t, tomlConfig), gateway.NewHandlerFactory(nil, nil, nil, lggr, limits.Factory{Logger: lggr}), lggr)
	require.NoError(t, err)
}
...
func TestGateway_CleanStartAndClose(t *testing.T) {
	...
	lggr := logger.Test(t)
	gateway, err := gateway.NewGatewayFromConfig(parseTOMLConfig(t, buildConfig("")), gateway.NewHandlerFactory(nil, nil, nil, lggr, limits.Factory{Logger: lggr}), lggr)
	require.NoError(t, err)
