...
func (m *InMemoryStoreManager) MarkTransactionsConfirmed(_ context.Context, nonce uint64, fromAddress common.Address) (confirmedTxs []*types.Transaction, unconfirmedTxIDs []uint64, err error) {
	if store, exists := m.InMemoryStoreMap[fromAddress]; exists {
		confirmedTxs, unconfirmedTxIDs, err = store.MarkTransactionsConfirmed(nonce)
		return
	}
