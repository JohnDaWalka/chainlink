...
func TestMultipleMetricsArePublished(t *testing.T) {
	...
	_ = orm.InsertLogsWithBlocks(ctx, []Log{}, []LogPollerBlock{
		{
			BlockNumber:    1,
...
func TestCountersAreProperlyPopulatedForWrites(t *testing.T) {
	...
	require.NoError(t, orm.InsertLogsWithBlocks(ctx, logs[10:15], []LogPollerBlock{
		{
	...
	// Insert 5 more logs with block
	require.NoError(t, orm.InsertLogsWithBlocks(ctx, logs[15:], []LogPollerBlock{
		{
			BlockHash:            utils.RandomBytes32(),
			BlockNumber:          15,
			BlockTimestamp:       time.Now(),
	...
	require.Error(t, orm.InsertLogsWithBlocks(ctx, logs, []LogPollerBlock{
		{
			BlockHash:      utils.RandomBytes32(),
			BlockTimestamp: time.Now(),
		},
