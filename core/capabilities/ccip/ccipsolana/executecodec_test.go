...
func Test_DecodingExecuteReport(t *testing.T) {
	...
	t.Run("decode on-chain execute report", func(t *testing.T) {
		...
		require.Equal(t, destGasAmount, binary.BigEndian.Uint32(msg.TokenAmounts[0].DestExecData))
	})
	...
	t.Run("decode Borsh encoded execute report", func(t *testing.T) {
		...
		require.Equal(t, binary.BigEndian.Uint32(originTokenAmount.DestExecData), executeReport.Message.TokenAmounts[0].DestGasAmount)
