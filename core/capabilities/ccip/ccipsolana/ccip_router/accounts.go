...
var BillingTokenConfigDiscriminator = [8]byte{191, 91, 1, 0, 93, 250, 239, 41}

func (obj BillingTokenConfig) MarshalWithEncoder(encoder *ag_binary.Encoder) (err error) {
	// Write account discriminator:
	err = encoder.WriteBytes(BillingTokenConfigDiscriminator[:], false)
	if err != nil {
		return err
	}
	// Serialize `Version` param:
...
func (obj *BillingTokenConfig) UnmarshalWithDecoder(decoder *ag_binary.Decoder) (err error) {
	// Read and check account discriminator:
	{
		...
		if !discriminator.Equal(BillingTokenConfigDiscriminator[:]) {
			return fmt.Errorf(
				"wrong discriminator: wanted %s, got %s",
				"[191 91 1 0 93 250 239 41]",
				fmt.Sprint(discriminator[:]))
		}
	}
	// Deserialize `Version`:
...
type TokenAdminRegistry struct {
	...
	PendingAdministrator *ag_solanago.PublicKey `bin:"optional"`
...
func (obj TokenAdminRegistry) MarshalWithEncoder(encoder *ag_binary.Encoder) (err error) {
	...
	// Serialize `PendingAdministrator` param (optional):
	{
		if obj.PendingAdministrator == nil {
	...
	// Serialize `TokenPoolProgram` param (optional):
	{
		if obj.TokenPoolProgram == nil {
			err = encoder.WriteBool(false)
			if err != nil {
				return err
			}
			...
			err = encoder.Encode(obj.TokenPoolProgram)
			if err != nil {
				return err
			}
		}
...
func (obj *TokenAdminRegistry) UnmarshalWithDecoder(decoder *ag_binary.Decoder) (err error) {
	...
	// Deserialize `PendingAdministrator` (optional):
	{
	...
	// Deserialize `TokenPoolProgram` (optional):
	{
		ok, err := decoder.ReadBool()
		if err != nil {
			return err
		...
		if ok {
			err = decoder.Decode(&obj.TokenPoolProgram)
			if err != nil {
				return err
			}
		}
