...
func Test_ServerRequest_MessageValidation(t *testing.T) {
	...
	t.Run("Register to workflow request errors", func(t *testing.T) {
		...
		assert.Equal(t, 2, len(dispatcher.msgs))
		assert.Equal(t, dispatcher.msgs[0].Error, types.Error_INTERNAL_ERROR)
		assert.Equal(t, dispatcher.msgs[0].ErrorMsg, "failed to register to workflow: an error")
		assert.Equal(t, dispatcher.msgs[1].Error, types.Error_INTERNAL_ERROR)
		assert.Equal(t, dispatcher.msgs[1].ErrorMsg, "failed to register to workflow: an error")
	})
	...
	t.Run("Unregister from workflow request errors", func(t *testing.T) {
		...
		assert.Equal(t, 2, len(dispatcher.msgs))
		assert.Equal(t, dispatcher.msgs[0].Error, types.Error_INTERNAL_ERROR)
		assert.Equal(t, dispatcher.msgs[0].ErrorMsg, "failed to unregister from workflow: an error")
		assert.Equal(t, dispatcher.msgs[1].Error, types.Error_INTERNAL_ERROR)
		assert.Equal(t, dispatcher.msgs[1].ErrorMsg, "failed to unregister from workflow: an error")
	})
