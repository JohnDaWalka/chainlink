...
func (m *oracleContext) addPlugin(info ocr3types.ReportingPluginInfo, plugin ocr3types.ReportingPlugin[[]byte],
	...
	if libOcr == nil {
		libOcr = NewFakeLibOCR(m.t, m.lggr, uint8(m.F), m.protocolRoundInterval)
...
func newFakeOracleFactoryFactory(ctx context.Context, t *testing.T, lggr logger.Logger, key ocr2key.KeyBundle, N int, F int, protocolRoundInterval time.Duration) *oracleFactoryFactory {
...
func (m *fakeOracle) Start(_ context.Context) error {
	plugin, info, err := m.args.ReportingPluginFactoryService.NewReportingPlugin(m.oracleContext.ctx, ocr3types.ReportingPluginConfig{
		F: m.oracleContext.F,
		N: m.oracleContext.N,
	})
	...
	if err = m.oracleContext.addPlugin(info, plugin, m.args); err != nil {
