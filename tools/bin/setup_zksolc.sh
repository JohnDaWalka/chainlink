...
ASSET_URL=$(curl --silent "$GITHUB_URL" | jq -r ".assets[] | select(.name == \"$ASSET_NAME2\") | .browser_download_url")
...
curl -L $ASSET_URL -o zksolc-bin

echo "Creating symbolic link"
sudo ln -s zksolc-bin /usr/local/bin/zksolc

echo "Giving permission to execute"
chmod +x /usr/local/bin/zksolc
